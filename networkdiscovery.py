...
def store_in_neo4j(local_ip, netmask, subnet, used_ips):
    with driver.session() as session:
        
        session.write_transaction(lambda tx: tx.run("""
            MERGE (s:Subnet {cidr: $cidr})
            SET s.netmask = $netmask, s.network = $network
        """, cidr=subnet.with_prefixlen, netmask=netmask, network=str(subnet.network_address)))
...
if __name__ == "__main__":
    ...
    print("\n[*] Storing active hosts in Neo4j...")
    store_in_neo4j(local_ip, netmask, subnet, used_ips)
